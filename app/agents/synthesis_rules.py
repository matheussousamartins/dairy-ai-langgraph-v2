...
SYNTHESIS_BASE_RULES: str = (
    ...
    "R5. Prosa coesa: reescreva em linguagem corrida e técnica. Proibido copiar a estrutura "
    "das evidências como headers, rótulos de seção (ex.: 'Adição de cloreto de cálcio:', "
    "'Inoculação de culturas láticas:'), bullets soltos, linhas iniciando no meio de uma "
    "frase ou metadados de chunk ('Trecho 1 — score 0.08'). Integre os dados em parágrafos "
    "fluidos onde cada ingrediente/etapa/parâmetro aparece dentro da narrativa técnica, "
    "não como título de bloco independente.\n"
...
FORMAT_INSTRUCTIONS: dict[str, str] = {
    ...
    QuestionType.GENERAL: (
        ...
        "Quando a resposta envolver múltiplos ingredientes, aditivos ou parâmetros, integre-os em "
        "parágrafos fluidos — nunca como lista de blocos com rótulo + dois-pontos (ex.: proibido "
        "'Cloreto de cálcio: X. Lisozima: Y.'). Use frases de transição ('além disso', 'em paralelo', "
        "'para controle de bacilos butíricos') para conectar os dados em narrativa coesa. "
...
def build_synthesis_prompt(
    *,
    ...
    return (
        ...
        f"RACIOCÍNIO OBRIGATÓRIO — escreva dentro das tags antes de responder:\n"
        f"<raciocinio>\n"
        f"1. Escopo da pergunta: [o que exatamente foi pedido — produto, parâmetro, conjunto de itens]\n"
        f"2. Itens nas evidências que respondem: [liste cada um]\n"
        f"3. Algum item do escopo ficou sem cobertura? [sim/não — se sim, o quê]\n"
        f"4. Tipo de resposta adequada: [prosa / etapas / tabela / frase direta]\n"
        f"</raciocinio>\n\n"
        f"Resposta final:"
    )
